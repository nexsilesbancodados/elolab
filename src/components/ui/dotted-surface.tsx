  ...
  useEffect(() => {
    ...
    for (let ix = 0; ix < AMOUNTX; ix++) {
      for (let iy = 0; iy < AMOUNTY; iy++) {
        ...
        if (isDark) {
          colors.push(0.6, 0.8, 0.7); // teal-ish for dark
        } else {
          colors.push(0.1, 0.45, 0.35); // dark teal for light
        }
      }
    }
    ...
    // Watch for theme class changes to re-create
    const observer = new MutationObserver(() => {
      // Theme changed — the effect will re-run via cleanup + re-mount
    });
